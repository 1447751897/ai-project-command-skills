---
name: ai-init
description: Use when the user writes /ai-init or wants to initialize a new software project from a requirement description. Clarify the project goal, scaffold AI_DEVELOPMENT_RULES.md and docs, generate or update requirements, development principles, technical decisions, handoff guide, roadmap, local development, deployment docs, project structure docs, upgrade history, command reference, decision log, frontend design direction, and current status before implementation. Also supports /ai-goal, /ai-goal --super, /ai-super, /ai-feature, /ai-change, /ai-fix, /ai-tech, /ai-deploy, /ai-handoff, /ai-roadmap, /ai-plan, /ai-status, /ai-continue, and /ai-upgrade for goal-mode automatic project execution, high-autonomy execution without ordinary confirmations, mid-project controlled requirement changes, feature recommendations when the user has no clear next step, cross-chat continuation, template upgrades, frontend design guidance, performance-aware design, and technical-selection gates.
---

# Ai-Init
## Claude Code Compatibility

This is the Claude Code alias of the 'init' workflow skill. Use the '/ai-init' slash command in Claude Code. The 'ai-' prefix avoids collisions with Claude Code built-in commands such as '/init', '/plan', and '/upgrade' while preserving the same document-first workflow semantics.
...
- `/ai-tech`: update `docs/04-tech-decisions.md` before or alongside any implementation.
- `/ai-deploy`: update `docs/08-deployment.md`, and update `docs/07-local-development.md` if local commands or env vars change.
- `/ai-handoff`: update `docs/05-handoff-guide.md`.
- `/ai-roadmap`: update `docs/06-roadmap.md`.
...
- `/ai-status`: inspect the current workspace, recent changes, docs, tests, and known pending work; update `docs/10-current-status.md` with a concise handoff-ready snapshot. Do not implement new work unless the user explicitly asks.
- `/ai-continue`: read `AI_DEVELOPMENT_RULES.md`, `docs/00_START_HERE.md`, `docs/10-current-status.md`, `docs/03-feature-changelog.md`, `docs/06-roadmap.md`, and `docs/05-handoff-guide.md`; summarize current state and continue only after identifying the next safe task.
