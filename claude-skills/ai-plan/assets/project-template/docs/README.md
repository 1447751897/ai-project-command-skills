...
13. 没有明确下一步时使用 `/ai-plan` 获取功能推荐和优先级建议。
14. 想让 AI 围绕一个最终目标自动推进时使用 `/ai-goal`。
15. 想让 AI 全程高自治推进、不等待普通确认时使用 `/ai-goal --super`，但关键决策必须记录到 `14-decision-log.md`。`/ai-super` 仅作为兼容别名。
