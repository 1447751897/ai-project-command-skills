...
| `engineering/02-development-principles.md` | 记录项目级开发原则、代码组织、API、数据、安全和质量规则 |

### 需求与计划

| 文档 | 作用 |
| --- | --- |
| `product/01-requirements-clarification.md` | 记录项目目标、用户、功能、业务流程和验收标准 |
| `product/06-roadmap.md` | 阶段路线图和每个阶段的验收标准 |

### 设计与技术

| 文档 | 作用 |
| --- | --- |
| `engineering/04-tech-decisions.md` | 记录技术选型的目的、原因、备选方案和影响 |
| `engineering/11-project-structure.md` | 项目整体结构、目录职责、模块地图和新增文件放置规则 |
| `product/15-frontend-design.md` | 记录产品形态、UI 风格参考、设计关键词、色彩、布局、组件和交互状态 |

### 开发过程

| 文档 | 作用 |
| --- | --- |
| `development/03-feature-changelog.md` | 记录开发过程中新增、变更、删除和修复的功能 |
| `development/10-current-status.md` | 当前开发进度快照，用于跨对话和交接接续 |
| `development/14-decision-log.md` | 高自治模式和重要技术/产品决策记录 |

### 运行与交付

| 文档 | 作用 |
| --- | --- |
| `operations/07-local-development.md` | 本地开发环境、启动命令、调试方式和常见问题 |
| `operations/08-deployment.md` | 部署环境、构建命令、部署步骤、上线验证和回滚方案 |

### 交接与使用说明

| 文档 | 作用 |
| --- | --- |
| `handoff/05-handoff-guide.md` | 给后续开发者或 AI 助手的接手指南 |
| `README.md` | 文档入口和父文件夹分类说明 |
| `maintenance/09-ai-project-start-prompt.md` | 新项目首次对话可直接复制的提示词 |
| `maintenance/12-upgrade-history.md` | Skill 或模板升级记录，说明新增文档和规则如何补齐 |
| `maintenance/13-command-reference.md` | 命令清单，说明每个命令的适用场景和作用 |

## 建议工作流

1. 新项目开始前，先放入 `AI_DEVELOPMENT_RULES.md` 和 `docs/` 模板。
2. 把 `maintenance/09-ai-project-start-prompt.md` 里的提示词发给 AI。
3. AI 先复述需求，再提出澄清问题。
4. 用户确认后，AI 填充 `docs/` 下的项目文档；涉及界面的项目必须先收集 UI 风格参考并更新 `product/15-frontend-design.md`。
5. 文档确认后，再进入开发。
6. 每次新增功能后更新 `development/03-feature-changelog.md`。
7. 每次技术选型变化后更新 `engineering/04-tech-decisions.md`。
...
11. 每次使用 `/ai-upgrade` 补齐项目新模板后更新 `maintenance/12-upgrade-history.md`；如果是更新本地已安装 skills，则运行本地 skills 更新器并提示重启 Claude Code。
...
16. 想让 AI 全程高自治推进、不等待普通确认时使用 `/ai-goal --super`，但关键决策必须记录到 `development/14-decision-log.md`。`/ai-super` 仅作为兼容别名。
