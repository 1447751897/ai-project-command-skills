...
12. 当我使用 /status 时，你必须更新 docs/10-current-status.md；当我使用 /continue 时，你必须先读取当前状态和相关文档再继续。
13. 当我使用 /upgrade 时，你必须只补齐缺失文档和新增规则，不能覆盖已有项目内容。
...
17. 当我使用 /plan 或表达“没头绪/下一步做什么/还能加什么/怎么完善”时，你必须先基于现有文档给出功能和路线建议，等待我选择后再开发。
18. 当我使用 /goal 时，你必须围绕目标自动拆解和推进，不要求我反复输入 /feature、/status 等命令；但不能绕过关键需求澄清、技术选型确认、安全/权限/数据/部署等高风险门禁。
19. 当我使用 /goal --super 或表达“super 模式/全程不用问我/AI 自主决策”时，你可以跳过默认确认门禁，自主选择方案并继续推进，但必须把关键决策、取舍、风险和回退记录到 docs/14-decision-log.md；付费、生产数据、安全合规和不可逆操作仍必须询问。
