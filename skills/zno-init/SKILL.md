---
name: zno-init
description: Use when the user writes /zno-init or wants to initialize a new software project from a requirement description. Clarify the project goal, scaffold AI_DEVELOPMENT_RULES.md and docs, generate or update requirements, development principles, technical decisions, handoff guide, roadmap, local development, deployment docs, project structure docs, upgrade history, command reference, decision log, frontend design direction, and current status before implementation. Also supports /zno-goal, /zno-goal --super, /zno-super, /zno-feature, /zno-change, /zno-fix, /zno-tech, /zno-deploy, /zno-handoff, /zno-roadmap, /zno-plan, /zno-status, /zno-continue, and /zno-upgrade for goal-mode automatic project execution, high-autonomy execution without ordinary confirmations, mid-project controlled requirement changes, feature recommendations when the user has no clear next step, cross-chat continuation, template upgrades, frontend design guidance, performance-aware design, and technical-selection gates.
---
...
/zno-evaluate <project idea or requirement>
/zno-review <optional scope or branch>
/zno-retro <optional phase or milestone>
...
## Project Evaluation (/zno-evaluate)

Use this workflow before `/zno-init` when the user has a project idea but has not yet committed to building it. The goal is to prevent wasted effort on projects that lack clear value, feasible scope, or viable ROI.

1. Trigger this flow when the user writes `/zno-evaluate`, says they have an idea but aren't sure if it's worth doing, or asks whether a project makes sense.
2. Use Socratic questioning to challenge the idea. Ask sequentially (not all at once):
...
3. After gathering answers (2-5 rounds of questions), output a structured evaluation:
...
4. Do not scaffold any project docs or write any code during evaluation.
5. If the conclusion is DO, suggest the user run `/zno-init` with the refined requirement statement.

## Retrospective (/zno-retro)

Use this workflow after completing a project phase, milestone, or the entire project. The goal is to extract lessons and prevent repeated mistakes.

1. Trigger this flow when the user writes `/zno-retro`, says a phase is done and wants to reflect, or asks what went well/badly.
2. Read the following docs to understand what was planned vs what happened:
...
5. If any lesson is generalizable (not project-specific), ask the user whether to append it to `AI_DEVELOPMENT_RULES.md` in the "可选项目定制区" section.
6. Update `docs/development/10-current-status.md` to reflect the retrospective was completed.
