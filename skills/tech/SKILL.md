---
name: tech
description: Use when the user writes /tech or requests a technical decision, architecture change, dependency choice, framework change, database/auth/service selection, performance strategy, or engineering tradeoff. Must present 2-3 options with pros, cons, performance impact, maintenance cost, risks, and a recommendation, then wait for user confirmation before implementation unless /goal --super mode is active; in /goal --super mode, record the autonomous decision in docs/14-decision-log.md. Also follows the shared project-kickoff document-first workflow and supports related prefixes /init, /goal, /goal --super, /super, /feature, /change, /fix, /tech, /deploy, /handoff, /roadmap, /plan, /status, /continue, and /upgrade.
---
