...
```text
/zno-feature 新增一个用户邀请功能，管理员可以邀请成员加入团队。
/zno-change 把邀请有效期从 24 小时改为 7 天。
/zno-fix 用户接受邀请后没有自动加入团队。
/zno-tech 将邀请邮件发送改为异步队列。
/zno-deploy 补充生产环境邮件服务的环境变量和回滚步骤。
/zno-status 总结当前开发进度并更新接续状态。
/zno-continue 读取当前状态并继续开发。
/zno-upgrade 补齐当前 Skill 新增的项目文档和规则。
/zno-plan 当没有头绪时，基于现有项目文档推荐下一步功能、优化方向和优先级。
/zno-goal 围绕一个最终目标自动推进开发，只有关键确认或阻塞时停下来。
/zno-goal --super 进入高自治模式，全程由 AI 自主决策并推进，后续输出决策过程。
```
